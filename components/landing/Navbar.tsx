...
export function Navbar({ navbar }: { navbar: LandingPageContent["navbar"] }) {
  ...
  return (
    <>
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-[color:var(--inverse-border-subtle)] bg-[color:var(--surface-inverse)] text-[color:var(--surface-inverse-foreground)]">
        <div className="mx-auto max-w-7xl px-6 py-6 md:py-8">
            ...
            <nav className="flex items-center justify-end gap-7 text-[11px] font-medium tracking-[0.22em] text-[color:var(--inverse-muted-foreground)]">
              {navbar.menu.slice(2, 4).map((item, idx) => (
                <Link
                  key={idx}
                  href={item.href}
                  className="uppercase text-(--inverse-muted-foreground) transition-colors transition-transform motion-reduce:transition-none hover:-translate-y-0.5 hover:-rotate-1 hover:text-[color:var(--surface-inverse-foreground)] motion-reduce:hover:translate-y-0 motion-reduce:hover:rotate-0"
                >
                  {item.label}
                </Link>
              ))}
          ...
          <div className="flex items-center justify-between md:hidden">
            ...
            <button
              ...
              className="inline-flex size-11 items-center justify-center rounded-full border border-[color:var(--inverse-border-subtle)] text-[color:var(--surface-inverse-foreground)] transition-transform active:scale-95"
            >
          ...
          >
            ...
            >
                ...
                {navbar.menu.map((item, idx) => (
                  ...
                  >
                    <Link
                      href={item.href}
                      onClick={closeMenu}
                      className="block border-b border-[color:var(--inverse-border-subtle)] pb-4 text-4xl font-black uppercase tracking-[-0.04em] text-[color:var(--surface-inverse-foreground)] transition-transform active:scale-[0.98]"
                    >
                      {item.label}
                    </Link>
                  </motion.div>
              ...
              >
                ...
                <p className="text-xs font-medium uppercase tracking-[0.22em] text-[color:var(--inverse-muted-foreground)]">
                  {navbar.brandName}
                </p>
              </motion.div>
            </motion.div>
